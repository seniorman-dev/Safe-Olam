...
class UnSafeConditionController extends GetxController {
  ...
  Future<File?> pickImageFromGallery() async{
    ...
    try {
      ...
      Get.snackbar('Error!', "$e", duration: Duration(seconds: 2), isDismissible: false, colorText: Colors.black, borderRadius: 10, backgroundColor: AppTheme.whiteColor, snackPosition: SnackPosition.BOTTOM, dismissDirection: DismissDirection.down);
    }
    update();
    return image;
  }
  ...
  Future saveSelectedImageToFirebase() async{
    if(isSelectedFromCamera) {
      if(image2 == null) {
        ...
        final destination = 'unsafe_condition_files/$fileName';
        try {
          ...
          firestore.collection('unsafe_condition').doc("${nameController.text}_${timeController.text}").set({
  ...
  Future<void> saveDetails(BuildContext context) async{
    try {
      if(isImageEmpty == true || nameController.text.isEmpty || reportController.text.isEmpty || locationController.text.isEmpty || timeController.text.isEmpty) {
        ...
        await firestore.collection('unsafe_condition').doc("${nameController.text}_${timeController.text}")
        .update({
        ...
        .whenComplete(() => showLoadingDialog(context).whenComplete(() => Get.snackbar('Success!', "your report has been submitted", duration: Duration(seconds: 2), isDismissible: false, colorText: Colors.black, borderRadius: 10, backgroundColor: AppTheme.whiteColor, snackPosition: SnackPosition.BOTTOM, dismissDirection: DismissDirection.down)));
